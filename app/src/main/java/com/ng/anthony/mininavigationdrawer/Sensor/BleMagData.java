...
public class BleMagData {
    ...
    public static List<String[]> accelertempSensorData = new ArrayList<String[]>();
    public static List<String[]> grivatySensorData = new ArrayList<String[]>();
...
//            "80:EC:CA:CD:11:B2", "80:EC:CA:CD:11:E6",};
    ...
    public static void acctemp_clear(){
        if(accelertempSensorData.size()!=0){
            accelertempSensorData.clear();
        }
    }

    public static void grivaty_clear(){
        if(grivatySensorData.size()!=0){
            grivatySensorData.clear();
        }
    }
    ...
    public static void addSensorData(String[] mData, String[] aData, String[] oData,
                                     String gData[], String stepcount,String accnorm,String magnorm, String[] atData, String[] grData, String captime){
        magneticSensorData.add(mData);
        accelerometerSensorData.add(aData);
        orientationSensorData.add(oData);
        accelertempSensorData.add(atData);
        grivatySensorData.add(grData);
        stepCount = stepcount;
...
//        }
    ...
    public static String getAcctempDataStr(){
        String data = "";
        if(accelertempSensorData.size() == 0) return "";
        for(int i = 0 ; i < accelertempSensorData.size() ; i++){
            String[] acc = accelertempSensorData.get(i);
            String one_detail = acc[0] + "," + acc[1] + "," + acc[2]+",";
            data = data + one_detail;
        }
        data = data.substring(0,data.length()-1);
    ...
    public static String getgrivatyDataStr(){
        String data = "";
        if(grivatySensorData.size() == 0) return "";
        for(int i = 0 ; i < grivatySensorData.size() ; i++){
            String[] gri = grivatySensorData.get(i);
            String one_detail = gri[0] + "," + gri[1] + "," + gri[2]+",";
            data = data + one_detail;
        }
        data = data.substring(0,data.length()-1);
        grivaty_clear();
    ...
    public static void setLocationResult(float X, float Y){
        boolean cons_flag = DataConstrain.getConstrainMode();
        if(!cons_flag){
            float[] res=DataConstrain.BoxConstrain0(X,Y);
...
//        locationResult[1] = res[1];
    ...
    public static void setLocationstate(int x){
        flag = x;
    }
